...
const LEVELS_INFO: Record<string, { color: string; emoji: string; description: string }> = {
    "Pilotage fragile": { color: "red", emoji: "🔴", description: "Structuration prioritaire" },
    "Bases présentes": { color: "yellow", emoji: "🟡", description: "Besoin d'encadrement" },
    "Organisation mature": { color: "green", emoji: "🟢", description: "DAF partiel pertinent" },
    "Pilotage avancé": { color: "blue", emoji: "🔵", description: "Rôle stratégique clair" },
};

export default function SharedResultsDAF({ score, level, fromName, onStartDiagnostic, onReadGuide }: SharedResultsProps) {
    const containerRef = useRef<HTMLElement>(null);
    const levelInfo = LEVELS_INFO[level] || LEVELS_INFO["Bases présentes"];
            ...
            <div className="relative z-10 max-w-2xl mx-auto text-center">
                ...
                <div className="shared-content space-y-6">
                    <h2 className="text-3xl md:text-4xl font-serif text-white">
                        Et vous, avez-vous besoin d&apos;un <span className="text-secondary">DAF</span> ?
                    </h2>

                    <p className="text-white/70 text-lg max-w-lg mx-auto">
                        Faites votre propre diagnostic pour savoir si votre PME a besoin
