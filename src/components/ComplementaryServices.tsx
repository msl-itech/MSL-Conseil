...
const COMPLEMENTARY_SERVICES = [
  {
    ...
    description:
      "Des indicateurs clairs pour piloter par les chiffres. Marges, budgets, centres de coûts, écarts : tout ce qu’il faut pour décider, pas pour deviner..",
  },
  {
    icon: (
      <svg
    ...
    description:
      "Une expertise stratégique à temps partiel pour structurer, cadrer et anticiper. Idéal pour les PME en croissance ou en transition.",
  },
  {
    icon: (
      <svg
    ...
    description:
      "Un regard expert sur ce qui est bien fait, ce qui freine, et ce qui doit être corrigé. Base parfaite pour décider de vos priorités opérationnelles.",
  },
  {
    icon: (
      <svg
    ...
    description:
      "Gagnez en fluidité entre services : compta, gestion, ventes, RH. On cartographie, on simplifie, on automatise intelligemment.",
  },
  {
    icon: (
      <svg
    ...
    description:
      "Clarifiez les rôles, les outils et les responsabilités. On pose les fondations d’un service fiable, autonome et aligné avec la direction.",
  },
  {
    icon: (
      <svg
    ...
    description:
      "Ne subissez pas les migrations, pilotez-les. Nous vous aidons à cadrer vos besoins, fiabiliser vos données et gérer les impacts.",
  },
];
...
export default function ComplementaryServices() {
  ...
  return (
    <section
      ref={container}
      className="relative w-full min-h-screen bg-primary text-white"
    >
      {/* Layout */}
          ...
          <div className="lg:sticky lg:top-0 w-full h-[50vh] lg:h-screen relative">
            ...
            <div className="absolute bottom-20 left-12 md:left-24 z-20 max-w-md">
              <h2 className="text-5xl md:text-7xl font-serif text-white mb-6 leading-[0.9]">
                Au-delà <br />{" "}
                <span className="text-secondary italic">du Conseil.</span>
              </h2>
        ...
        <div className="flex flex-col justify-center px-12 md:px-24 py-24 relative z-20 bg-primary lg:bg-transparent">
          <div className="mb-16">
            ...
            <p className="text-white/60 text-lg font-light mb-8">
              Un accompagnement à 360° autour de vos outils, vos flux et vos
              décisions.
            </p>
            ...
            <div className="space-y-4">
              {COMPLEMENTARY_SERVICES.map((service, index) => (
                <div
                  key={index}
                  className="service-item group flex items-start gap-6 py-6 border-b border-white/10 hover:border-white/30 transition-colors duration-300"
                >
